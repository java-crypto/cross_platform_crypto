...
func main() {
  ...
  fmt.Println("JWT JWS PS256 RSA with RSASSA-PSS + MGF1 with SHA-256 padding and SHA-256 hashing")

  jwsAlgorithm := string("PS256")
  ...
  subject := string("JWT PS256 signature")
...
func JwsSignatureToString(jwsAlgorithm string, privateKeyByte []byte, payload string)(string) {
  privateKey,e:=Rsa.ReadPrivate(privateKeyByte)
	...
	token,err := jose.Sign(payload,jose.PS256, privateKey)
	if(err==nil) {
		//go use token
		return token
	}
