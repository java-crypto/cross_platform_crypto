<?php
function getFixedIv8Byte()
{ // ### security warning - never use this in production ###
    return "\x00\x00\x00\x00\x00\x00\x00\x00";}
function base64Encoding($input){return base64_encode($input);}
function base64Decoding($input){return base64_decode($input);}
function pkcs7Padding($data, $blocklen) {
    $len = strlen($data);
    $padding = $blocklen - ($len % $blocklen);
    $data .= str_repeat(chr($padding),$padding);
    return $data;
}
function pkcs7Unpadding($data, $blocklen) {
    $packing = ord($data{strlen($data) - 1});
    if($packing and ($packing < $blocklen)){
        for($P = strlen($data) - 1; $P >= strlen($data) - $packing; $P--){
            if(ord($data{$P}) != $packing){
                $packing = 0;
            }
        }
    }
    return substr($data,0,strlen($data) - $packing);
}
function printData($name, $data) {
    echo $name . ' length: ' . strlen($data)
        . ' hex: ' . bin2hex($data)
        . ' base64: '. base64Encoding($data)
        . PHP_EOL;
}
function printStringData($name, $data) {
    echo $name . ' length: ' . strlen($data)
        . ' hex: ' . bin2hex($data)
        . ' string: ' . $data . '#'
        . PHP_EOL;
    $data = rtrim($data); // needed for strip off the x00s at the end
    echo $name . ' length: ' . strlen($data)
        . ' hex: ' . bin2hex($data)
        . ' string: ' . $data . '#'
        . PHP_EOL;
}

function mcrypt_des_ecb_zeroPadding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_DES, $key, $plaintext, MCRYPT_MODE_ECB);
    $name = 'DES ECB Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_DES, $key, $ciphertext, MCRYPT_MODE_ECB);
    printStringData(('Dec ' . $name), $decryptedtext);
}

function mcrypt_des_ecb_pkcs7Padding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_DES, $key, pkcs7Padding($plaintext, 8), MCRYPT_MODE_ECB);
    $name = 'DES ECB Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_DES, $key, $ciphertext, MCRYPT_MODE_ECB);
    printStringData(('Dec ' . $name), pkcs7Unpadding($decryptedtext, 8));
}

function mcrypt_des_cbc_zeroPadding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_DES, $key, $plaintext, MCRYPT_MODE_CBC, getFixedIv8Byte());
    $name = 'DES CBC Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_DES, $key, $ciphertext, MCRYPT_MODE_CBC, getFixedIv8Byte());
    printStringData(('Dec ' . $name), $decryptedtext);
}

function mcrypt_des_cbc_pkcs7Padding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_DES, $key, pkcs7Padding($plaintext, 8), MCRYPT_MODE_CBC, getFixedIv8Byte());
    $name = 'DES CBC Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_DES, $key, $ciphertext, MCRYPT_MODE_CBC, getFixedIv8Byte());
    printStringData(('Dec ' . $name), pkcs7Unpadding($decryptedtext, 8));
}

function mcrypt_des_ctr_noPadding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_DES, $key, $plaintext, 'ctr', getFixedIv8Byte());
    $name = 'DES CTR Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_DES, $key, $ciphertext, 'ctr', getFixedIv8Byte());
    printStringData(('Dec ' . $name), $decryptedtext);
}

function mcrypt_tdes_ecb_zeroPadding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_3DES, $key, $plaintext, MCRYPT_MODE_ECB);
    $name = 'TDES ECB Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_3DES, $key, $ciphertext, MCRYPT_MODE_ECB);
    printStringData(('Dec ' . $name), $decryptedtext);
}

function mcrypt_tdes_ecb_pkcs7Padding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_3DES, $key, pkcs7Padding($plaintext, 8), MCRYPT_MODE_ECB);
    $name = 'TDES ECB Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_3DES, $key, $ciphertext, MCRYPT_MODE_ECB);
    printStringData(('Dec ' . $name), pkcs7Unpadding($decryptedtext, 8));
}

function mcrypt_tdes_cbc_zeroPadding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_3DES, $key, $plaintext, MCRYPT_MODE_CBC, getFixedIv8Byte());
    $name = 'TDES CBC Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_3DES, $key, $ciphertext, MCRYPT_MODE_CBC, getFixedIv8Byte());
    printStringData(('Dec ' . $name), $decryptedtext);
}

function mcrypt_tdes_cbc_pkcs7Padding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_3DES, $key, pkcs7Padding($plaintext, 8), MCRYPT_MODE_CBC, getFixedIv8Byte());
    $name = 'TDES CBC Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_3DES, $key, $ciphertext, MCRYPT_MODE_CBC, getFixedIv8Byte());
    printStringData(('Dec ' . $name), pkcs7Unpadding($decryptedtext, 8));
}

function mcrypt_tdes_ctr_noPadding($key, $plaintext) {
    $ciphertext = mcrypt_encrypt(MCRYPT_3DES, $key, $plaintext, 'ctr', getFixedIv8Byte());
    $name = 'TDES CTR Key ' . strlen($key) . ' B Pt ' . strlen($plaintext) . ' B';
    printData(('Enc ' . $name), $ciphertext);
    $decryptedtext = mcrypt_decrypt(MCRYPT_3DES, $key, $ciphertext, 'ctr', getFixedIv8Byte());
    printStringData(('Dec ' . $name), $decryptedtext);
}

echo 'MCRYPT example program for PHP up to 5.6.x' . PHP_EOL;
echo 'This program encrypts and decrypts strings of 15/16/17/31/32/33 bytes length.' . PHP_EOL;
echo 'The encryption algorithms are DES and TripleDES (TDES)' . PHP_EOL;
echo 'The tested key lengths are 8 (DES) and 24 (TDES) bytes, modes are ECB/CBC/CTR' . PHP_EOL;
echo 'All tests are done with ZERO PADDING (MCRYPT default) and PKCS7 padding' . PHP_EOL;
echo 'except CTR mode that has no padding' . PHP_EOL;

$plaintexts = array("123456789012345", "1234567890123456", "12345678901234567",
    "1234567890123456789012345678901", "12345678901234567890123456789012", "123456789012345678901234567890123");

echo PHP_EOL . 'DES ECB encryption with 8 key and 15/16/17/31/32/33 plaintext zero padding' . PHP_EOL;
$key = "12345678";
foreach ($plaintexts as $plaintext) {
    mcrypt_des_ecb_zeroPadding($key, $plaintext);
}

echo PHP_EOL . 'DES ECB encryption with 8 key and 15/16/17/31/32/33 plaintext PKCS7 padding' . PHP_EOL;
$key = "12345678";
foreach ($plaintexts as $plaintext) {
    mcrypt_des_ecb_pkcs7Padding($key, $plaintext);
}

echo PHP_EOL . 'DES CBC encryption with 8 key and 15/16/17/31/32/33 plaintext zero padding' . PHP_EOL;
$key = "12345678";
foreach ($plaintexts as $plaintext) {
    mcrypt_des_cbc_zeroPadding($key, $plaintext);
}

echo PHP_EOL . 'DES CBC encryption with 8 key and 15/16/17/31/32/33 plaintext PKCS7 padding' . PHP_EOL;
$key = "12345678";
foreach ($plaintexts as $plaintext) {
    mcrypt_des_cbc_pkcs7Padding($key, $plaintext);
}

echo PHP_EOL . 'DES CTR encryption with 8 key and 15/16/17/31/32/33 plaintext no padding' . PHP_EOL;
$key = "12345678";
foreach ($plaintexts as $plaintext) {
    mcrypt_des_ctr_noPadding($key, $plaintext);
}

echo PHP_EOL . 'TDES ECB encryption with 24 key and 15/16/17/31/32/33 plaintext zero padding' . PHP_EOL;
$key = "123456789012345678901234";
foreach ($plaintexts as $plaintext) {
    mcrypt_tdes_ecb_zeroPadding($key, $plaintext);
}

echo PHP_EOL . 'TDES ECB encryption with 24 key and 15/16/17/31/32/33 plaintext PKCS7 padding' . PHP_EOL;
$key = "123456789012345678901234";
foreach ($plaintexts as $plaintext) {
    mcrypt_tdes_ecb_pkcs7Padding($key, $plaintext);
}

echo PHP_EOL . 'TDES CBC encryption with 8 key and 15/16/17/31/32/33 plaintext zero padding' . PHP_EOL;
$key = "123456789012345678901234";
foreach ($plaintexts as $plaintext) {
    mcrypt_tdes_cbc_zeroPadding($key, $plaintext);
}

echo PHP_EOL . 'TDES CBC encryption with 8 key and 15/16/17/31/32/33 plaintext PKCS7 padding' . PHP_EOL;
$key = "123456789012345678901234";
foreach ($plaintexts as $plaintext) {
    mcrypt_tdes_cbc_pkcs7Padding($key, $plaintext);
}

echo PHP_EOL . 'TDES CTR encryption with 8 key and 15/16/17/31/32/33 plaintext no padding' . PHP_EOL;
$key = "123456789012345678901234";
foreach ($plaintexts as $plaintext) {
    mcrypt_tdes_ctr_noPadding($key, $plaintext);
}
?>