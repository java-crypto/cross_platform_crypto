<?php
function rsaOaepSha1AesGcm256HybridStringEncryption($publicKey, $plaintext) {
    // generate random key
...
function rsaOaepSha1AesGcm256HybridStringDecryption ($privateKey, $completeCiphertext) {
    list($encryptionkeyReceived, $iv, $encryptedData, $gcmTag) = explode(':', $completeCiphertext, 4);
    $encryptionkeyReceived = base64Decoding($encryptionkeyReceived);
    $iv = base64Decoding($iv);
    $encryptedData = base64Decoding($encryptedData);
    $gcmTag = base64Decoding($gcmTag);
    // decrypt the encryption key with the rsa private key
    $decryptionKey = rsaDecryptionOaepSha1($privateKey, $encryptionkeyReceived);
    return aesGcmDecrypt($decryptionKey, $iv, $encryptedData, $gcmTag);
}
...
function aesGcmDecrypt($key, $nonce, $encryptedData, $gcmTag)
{
    //list($nonce, $encryptedData, $tag) = explode(':', $data, 3);
    return openssl_decrypt($encryptedData, 'aes-256-gcm', $key, OPENSSL_RAW_DATA, $nonce, $gcmTag);
}
...
echo 'RSA AES GCM hybrid encryption' . PHP_EOL;

echo PHP_EOL . 'uses AES GCM 256 random key for the plaintext encryption' . PHP_EOL;
...
$completeCiphertextBase64 = rsaOaepSha1AesGcm256HybridStringEncryption($publicKeyLoad, $dataToEncryptString);
...
$decryptedtext = rsaOaepSha1AesGcm256HybridStringDecryption($privateKeyLoad, $completeCiphertextReceivedBase64);
