package main

import (
	"crypto"
	"crypto/rand"
	"crypto/rsa"
  "crypto/sha1"
	"encoding/base64"
	"fmt"
  "crypto/x509"
  "encoding/pem"
)

func main() {

  fmt.Println("RSA 2048 encryption OAEP SHA-1 string")

  dataToEncryptString := string("The quick brown fox jumps over the lazy dog")
  fmt.Println("plaintext: " + dataToEncryptString)
  
  // usually we would load the private and public key from a file or keystore
  // here we use hardcoded keys for demonstration - don't do this in real programs

  // encryption
  fmt.Println("\n* * * encrypt the plaintext with the RSA public key * * *")
  // load public key
  publicKeyByte := []byte(LoadRsaPublicKeyPem());
  fmt.Println("public key loaded")
  ciphertextBase64 := RsaEncryptionOaepSha1ToBase64(publicKeyByte, dataToEncryptString)
  fmt.Println("ciphertextBase64: " + ciphertextBase64)
  
  fmt.Println("\n* * * decrypt the ciphertext with the RSA private key * * *")
  // load private key
  privateKeyByte := []byte(LoadRsaPrivateKeyPem());
  fmt.Println("private key loaded")
  ciphertextReceivedBase64 := string(ciphertextBase64)
  decryptedtext := string(RsaDecryptionOaepSha1FromBase64(privateKeyByte, ciphertextReceivedBase64))
  fmt.Println("decryptedtext: " + decryptedtext)
}

func RsaEncryptionOaepSha1ToBase64(publicKeyByte []byte, dataToEncrypt string) (string){
  block, _ :=pem.Decode(publicKeyByte)
  parseResultPublicKey,err := x509.ParsePKIXPublicKey(block.Bytes)
  publicKey := parseResultPublicKey.(*rsa.PublicKey)
  encryptedBytes, err := rsa.EncryptOAEP(
	  sha1.New(),
	  rand.Reader,
	  publicKey,
	  []byte(dataToEncrypt),
	  nil)
  if err != nil {
	  panic(err)
  }
  return Base64Encoding(encryptedBytes)
}

func RsaDecryptionOaepSha1FromBase64(privateKeyByte []byte, ciphertextBase64 string)(string){
  block, _ := pem.Decode(privateKeyByte)
  parseResult, _ := x509.ParsePKCS8PrivateKey(block.Bytes)
  privateKey := parseResult.(*rsa.PrivateKey)
  decryptedBytes, err := privateKey.Decrypt(nil, Base64Decoding(ciphertextBase64), &rsa.OAEPOptions{Hash: crypto.SHA1})
  if err != nil {
	  panic(err)
  }
  return string(decryptedBytes)
}

func LoadRsaPrivateKeyPem()([]byte) {
  return []byte(`-----BEGIN PRIVATE KEY-----

3PIW4/CddNs8mCSBOqTnoaxh
-----END PRIVATE KEY-----`)
}

func LoadRsaPublicKeyPem()([]byte) {
  return []byte(`-----BEGIN PUBLIC KEY-----

QwIDAQAB
-----END PUBLIC KEY-----`)
}  

func Base64Encoding(input []byte)(string) {
  return base64.StdEncoding.EncodeToString(input)
}

func Base64Decoding(input string)([]byte) {
  data, err := base64.StdEncoding.DecodeString(input)
	if err != nil {
		return data
	}
  return data
}
