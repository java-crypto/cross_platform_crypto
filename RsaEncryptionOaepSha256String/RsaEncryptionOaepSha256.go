...
func main() {
  ...
  fmt.Println("RSA 2048 encryption OAEP SHA-256 string")
  ...
  ciphertextBase64 := RsaEncryptionOaepSha256ToBase64(publicKeyByte, dataToEncryptString)
  ...
  decryptedtext := string(RsaDecryptionOaepSha256FromBase64(privateKeyByte, ciphertextReceivedBase64))
  fmt.Println("decryptedtext: " + decryptedtext)
}

func RsaEncryptionOaepSha256ToBase64(publicKeyByte []byte, dataToEncrypt string) (string){
  ...
  return Base64Encoding(encryptedBytes)
}

func RsaDecryptionOaepSha256FromBase64(privateKeyByte []byte, ciphertextBase64 string)(string){
  block, _ := pem.Decode(privateKeyByte)
  parseResult, _ := x509.ParsePKCS8PrivateKey(block.Bytes)
  privateKey := parseResult.(*rsa.PrivateKey)
  decryptedBytes, err := privateKey.Decrypt(nil, Base64Decoding(ciphertextBase64), &rsa.OAEPOptions{Hash: crypto.SHA256})
  if err != nil {
	  panic(err)
  }
