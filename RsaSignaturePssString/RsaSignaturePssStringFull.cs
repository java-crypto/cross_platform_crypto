...
class RrsaSignatureString {
	public static void Main() {
		...
		try {
			...
			string privateRsaKeyXml = loadRsaPrivateKeyXml();
			Console.WriteLine("used private key:\n" + privateRsaKeyXml);
			string signatureBase64 = rsaSignPssToBase64(privateRsaKeyXml, dataToSign);
			Console.WriteLine("\nsignature (Base64): " + signatureBase64);

			Console.WriteLine("\n* * * verify the signature against the plaintext with the RSA public key * * *");
			string publicRsaKeyXml = loadRsaPublicKeyXml();
			Console.WriteLine("used public key:\n" + publicRsaKeyXml);
			bool signatureVerified = rsaVerifySignaturePssFromBase64(publicRsaKeyXml, dataToSign, signatureBase64);
