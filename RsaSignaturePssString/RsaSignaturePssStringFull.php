<?php
// uses phpseclib version 3
include 'vendor/autoload.php';
use phpseclib3\Crypt\RSA;
use phpseclib3\Crypt\PublicKeyLoader;

function rsaSignPssToBase64 ($privateKey, $dataToSign) {
    $rsaSign = PublicKeyLoader::load($privateKey);
    $rsaSign = $rsaSign
        ->withPadding(RSA::SIGNATURE_PSS)
        ->withHash('sha256')
        ->withMGFHash('sha256')
        ->withSaltLength(32);
    return base64_encode($rsaSign->sign($dataToSign));
}

function rsaVerifySignaturePssFromBase64 ($publicKey, $dataToSign, $signatureBase64) {
    $rsaVerify = PublicKeyLoader::load($publicKey)
        ->withHash('sha256')
        ->withMGFHash('sha256')
        ->withSaltLength(32);
    $ok = $rsaVerify->verify($dataToSign, base64_decode($signatureBase64));
    if ($ok == 1) {
        return true;
    } elseif ($ok == 0) {
        return false;
    } else {
        return false;
    }
    return false;
}

function loadRsaPrivateKeyPem() {
    // this is a sample key - don't worry !
    return '
-----BEGIN PRIVATE KEY-----

3PIW4/CddNs8mCSBOqTnoaxh
-----END PRIVATE KEY-----
';
}

function loadRsaPublicKeyPem() {
    // this is a sample key - don't worry !
    return '
-----BEGIN PUBLIC KEY-----

QwIDAQAB
-----END PUBLIC KEY-----
';
}

echo 'RSA signature PSS string' . PHP_EOL;

$dataToSign = "The quick brown fox jumps over the lazy dog";
echo 'dataToSign: ' . $dataToSign . PHP_EOL;

// usually we would load the private and public key from a file or keystore
// here we use hardcoded keys for demonstration - don't do this in real programs

echo PHP_EOL . '* * * sign the plaintext with the RSA private key * * *' . PHP_EOL;
$privateRsaKeyPem = loadRsaPrivateKeyPem();
echo 'used private key:' . $privateRsaKeyPem . PHP_EOL;
$signatureBase64 = rsaSignPssToBase64($privateRsaKeyPem, $dataToSign);
echo 'signature (Base64): ' . $signatureBase64 . PHP_EOL;

echo PHP_EOL . '* * * verify the signature against the plaintext with the RSA public key * * *' . PHP_EOL;
$publicRsaKeyPem = loadRsaPublicKeyPem();
echo 'used public key:' . $publicRsaKeyPem . PHP_EOL;
$signatureVerified = rsaVerifySignaturePssFromBase64($publicRsaKeyPem, $dataToSign, $signatureBase64);
if ($signatureVerified == 1) {
    echo 'signature (Base64) verified: ' . 'true'. PHP_EOL;
} else {
    echo 'signature (Base64) verified: ' . 'false'. PHP_EOL;
}
?>
