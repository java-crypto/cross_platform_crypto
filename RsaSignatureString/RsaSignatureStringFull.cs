...
class RrsaSignatureString {
    ...
    try {
        ...
        string signatureBase64 = rsaSignToBase64(privateRsaKeyPem, dataToSign);
  ...
  private static string rsaSignToBase64(string rsaPrivateKey, byte[] dataToSign) {
      try {
        // getting the rsa private key from xml
        RSACryptoServiceProvider RSAalg = new RSACryptoServiceProvider(2048);
			  RSAalg.PersistKeyInCsp = false;
			  RSAalg.FromXmlString(rsaPrivateKey);
			  return Base64Encoding(RSAalg.SignData(dataToSign, new SHA256CryptoServiceProvider()));
